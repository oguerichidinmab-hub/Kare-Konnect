...
const CounselorsScreen = () => {
  ...
  const FilterBadge = ({ active, label, onClick }: { active: boolean, label: string, onClick: () => void }) => (
    <Badge 
      className={cn(
      ...
      <Modal isOpen={showFilterModal} onClose={() => setShowFilterModal(false)} title="Filter Counselors">
        <div className="space-y-6 max-h-[70vh] overflow-y-auto pb-4 pr-2">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {['All', 'Anxiety', 'Depression', 'Trauma', 'Grief', 'Disability Support'].map(exp => (
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {['All', 'EMDR', 'CBT', 'Mindfulness', 'Person-Centered Therapy', 'Family Systems'].map(spec => (
